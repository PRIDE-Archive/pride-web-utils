...
/**
 * Default {@code RestErrorResolver} implementation that converts discovered exceptions to {@link
 * RestError} instances
 *
 * @author Rui Wang
 * @version $Id$
...
public class DefaultRestErrorResolver implements RestErrorResolver {
  ...
  /**
   * Set a map of mappings between exception classes and rest error instances Please note: these
   * rest error instances are only going to be used as template, They will not be modified.
   *
   ...
   * @param ex exception will be used as throwable parameter, and also if the developer message is
   *     null, exception message will be used instead
  ...
  private RestError copyRestErrorTemplate(RestError restErrorTemplate, Exception ex) {
    ...
    return builder
        .setStatus(restErrorTemplate.getStatus())
        .setCode(restErrorTemplate.getCode())
        .setMessage(restErrorTemplate.getMessage())
        .setDeveloperMessage(
            restErrorTemplate.getDeveloperMessage() == null
                ? ex.getMessage()
                : restErrorTemplate.getDeveloperMessage())
        .setMoreInfoUrl(
            restErrorTemplate.getMoreInfoUrl() == null
                ? defaultMoreInfoUrl
                : restErrorTemplate.getMoreInfoUrl())
        .setThrowable(ex)
        .build();
  }
}
