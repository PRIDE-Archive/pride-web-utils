 ...
 *
 * <p><a
 * href="http://www.stormpath.com/blog/spring-mvc-rest-exception-handling-best-practices-part-1">
 * Spring MVC Rest Exception Handling Best Practices.</a></p>
 *
 * <h2>JSON Rendering</h2>
 *
 * This implementation comes pre-configured with Spring's typical default {@link
 * org.springframework.http.converter.HttpMessageConverter} instances; JSON will be enabled
 * automatically if Jackson is in the classpath. If you want to match the JSON representation shown
 * in the article above (recommended) but do not want to use Jackson (or the Spring's default
 * Jackson config), you will need to {@link
 * #setMessageConverters(org.springframework.http.converter.HttpMessageConverter[]) configure} a
 * different JSON-capable {@link org.springframework.http.converter.HttpMessageConverter}.
 *
 * @author Rui Wang
 * @version $Id$
 */
public class RestExceptionHandler extends AbstractHandlerExceptionResolver
  ...
  protected ModelAndView doResolveException(
    ...
    try {
      ...
      logger.error(
          "Acquiring ModelAndView for Exception [" + ex + "] resulted in an exception.", e);
    }

    return mav;
  }

  private ModelAndView getModelAndView(ServletWebRequest webRequest, RestError restError)
      throws Exception {
  ...
  private ModelAndView buildResponseBody(Object body, ServletWebRequest webRequest)
      throws IOException {
