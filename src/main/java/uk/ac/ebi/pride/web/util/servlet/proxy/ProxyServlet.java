package uk.ac.ebi.pride.web.util.servlet.proxy;
...
/**
 * ProxyServlet from http://edwardstx.net/wiki/attach/HttpProxyServlet/ProxyServlet.java (This seems
 * to be a derivative of Noodle -- http://noodle.tigris.org/) Patched to skip "Transfer-Encoding:
 * chunked" headers, and avoid double slashes in proxied URL's.
 *
 * <p>Modified for use in PRIDE projects by
 *
...
public class ProxyServlet extends HttpServlet {
  ...
  /**
   * Setting that allows removing the initial path from client. Allows specifying /twitter/* as
   * synonym for twitter.com.
   */
   ...
   *
   * @param httpServletRequest The {@link javax.servlet.http.HttpServletRequest} object passed in by
   *     the servlet engine representing the client request to be proxied
  ...
  public void doGet(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse)
    ...
    debug(
        "GET Request URL: " + httpServletRequest.getRequestURL(),
  ...
  public void doPost(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse)
    ...
    String destinationUrl = this.getProxyURL(httpServletRequest);
    debug(
        "POST Request URL: " + httpServletRequest.getRequestURL(),
  ...
  private void handleContentPost(
    ...
    if (contentType.startsWith("text/x-gwt-rpc")) {
      ...
      String serverUrl =
          stringProxyHost + ((intProxyPort != 80) ? ":" + intProxyPort : "") + stringPrefixPath;
      postContent = postContent.replace(clientUrl, serverUrl);
    }

    String encoding = httpServletRequest.getCharacterEncoding();
    debug(
        "POST Content Type: " + contentType + " Encoding: " + encoding, "Content: " + postContent);
  ...
  /**
   * Executes the {@link org.apache.commons.httpclient.HttpMethod} passed in and sends the proxy
   * response back to the client via the given {@link javax.servlet.http.HttpServletResponse}
   *
   * @param httpMethodProxyRequest An object representing the proxy request to be made
   * @param httpServletResponse An object by which we can send the proxied response back to the
   *     client
   * @throws java.io.IOException Can be thrown by the {@link
   *     org.apache.commons.httpclient.HttpClient}.executeMethod
   * @throws javax.servlet.ServletException Can be thrown to indicate that another error has
   *     occurred
   */
  ...
  /**
   * A highly performant ungzip implementation. Do not refactor this without taking new timings. See
   * ElementTest in ehcache for timings
   *
  ...
  private String getProxyURL(HttpServletRequest httpServletRequest) {
    ...
    if (path == null || path.isEmpty()) {
      // simply use whatever servlet path that was part of the request as opposed to getting a
      // preset/configurable proxy path
